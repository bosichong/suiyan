...
def rmblog():
    ...
    for i in os.listdir(BLOGPAGES):  # os.listdir(path_data)#返回一个列表，里面是当前目录下面的所有东西的相对路径
        ...
        if os.path.isfile(file_data) and file_data.endswith(".html"):
            os.remove(file_data)
            # print(file_data)
...
def create_main_html(str):
    ...
    blogdescription = mainbfs.find(
        name='div', attrs={"class": "blog-description"})
    ...
    blogfooter = mainbfs.find(name='footer', attrs={
                              "class": "footer"})  # blognav
...
def create_list_html(indexhtml, i, ps, cs):
    ...
    if i < ps:
        ...
        for p in range(cs * i, maxp):
            ...
            # print(blogdata[p])
            # 组装每一页的博文HTML代码
            bdata += markdown(loadcode(os.path.join(ARTICLES_DIR,
                                                    blogdata[p]["url"] + '.md')))
        # 组装分页代码
        ...
        p_prev = '<a href="' + prevurl + \
            '"><button type="button" class="prev btn btn-primary" >Previous</button></a>'
        p_next = '<a href="' + nexturl + \
            '"><button type="button" class="next btn btn-primary" >Next</button></a>'
        p_page = '<button type="button" class="page btn btn-primary">' + \
            str(i) + ' / ' + str(ps) + ' </button>'
        ...
        datalist = BeautifulSoup('<div class="data-list list-group">' +
                                 str(homelistbfs) + '</div>', 'html.parser')  # 首页博文列表
    ...
    for l in data:
        ...
        for j in l["data"]:
            ...
            lihtmlstr += '<li class="list-group-item"><a href="' + burl + '.html" target="_blank">' + \
                ...
                j["time"] + '</span></li>'
        ulhtmstr = '<ul class="car-monthlisting list-group" style="display: block;">' + \
            lihtmlstr + '</ul>'
...
def create_blog_html(mainhtml, blog):
    ...
    # 外层代码
    whtml = '<div class="container-fluid" id="rt">' + \
        mdbfs + githtml + '<div class="blog-article"></div>'
